...
def init_fuse_as_passthrough(conv1x1, C: int, S: int, which: int = -1):

    with torch.no_grad():
        conv1x1.weight.zero_()
        if conv1x1.bias is not None:
            conv1x1.bias.zero_()
        start = (which % S) * C
        for o in range(C):
            conv1x1.weight[o, start + o, 0] = 1.0
...
class SqueezeExcitation(nn.Module): #xi
    ...
    def forward(self, x):
        b, c, _ = x.size()
        y = self.global_avg_pool(x).view(b, c)  # Squeeze: [B, C, T] -> [B, C]
        y = self.fc1(y)
        y = self.relu(y)
        y = self.fc2(y)
        y = self.sigmoid(y).view(b, c, 1)  # Excitation: [B, C] -> [B, C, 1]
        return x * y  # Reweight
...
class TemporalBlock(BaseTCN):
    def __init__(
        ...
        self.causal = causal
        self.se = SqueezeExcitation(channel=n_outputs, reduction=4) #xi
...
class TCN(BaseTCN):
    def __init__(
        ...
        if self.use_skip_connections:
            S = len(num_channels)       
            C = num_channels[-1]       
            self.fuse_conv = nn.Conv1d(in_channels=S*C, out_channels=C, kernel_size=1, bias=True)
            init_fuse_as_passthrough(self.fuse_conv, C=C, S=S, which=-1)  
    ...
    def forward(
        ...
        if self.use_skip_connections:
            ...
            #     if in_buffers:
            #         layer_in_buffers = in_buffers[ 1*index: ]
            #     else:
            ...
            for index, layer in enumerate(self.network):
                ...
                feats.append(x)
            cat = torch.cat(feats, dim=1) / 2.0  
            fused = self.fuse_conv(cat)
            x = self.activation_skip_out(fused)
        else:
            for index, layer in enumerate( self.network ):
